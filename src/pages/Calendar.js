...
function Calendar() {
  ...
  useEffect(() => {
    async function fetchIdeas() {
      ...
      const processedIdeas = fetchedIdeas
        .map((idea) => {
      ...
      processedIdeas.sort(
        (a, b) =>
          parseFloat(b.acceptancePercentage) -
          parseFloat(a.acceptancePercentage)
      );
      ...
      {selectedIdea && (
        <div className="mt-6 border rounded p-4 shadow-lg bg-white">
          ...
          {selectedIdea.imagePreview && (
            <img
              src={selectedIdea.imagePreview}
              alt="Preview"
              className="w-full max-h-60 object-cover rounded mb-2"
            />
          )}
          <p>
            <strong>Date:</strong> {selectedIdea.date?.toDate().toDateString()}
          </p>
          <p>
            <strong>Location:</strong> {selectedIdea.location.latitude},{' '}
            {selectedIdea.location.longitude}
          </p>
          <p>
            <strong>Description:</strong> {selectedIdea.description}
          </p>
          <p>
            <strong>Acceptance:</strong> {selectedIdea.acceptancePercentage}%
          </p>
          <div className="mt-2 flex gap-2 flex-wrap">
            {selectedIdea.tags.map((tag, i) => (
              <span key={i} className="px-2 py-1 bg-blue-200 rounded text-sm">
                {tag}
              </span>
            ))}
          </div>
